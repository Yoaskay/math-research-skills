...
6. **Broad milestone pacing** — Keeps the initial map, then advances by short, meaningful takeaways within broad milestones. Ordinary follow-ups, answers, and repairs require little scrolling; larger milestones span turns without atomizing the lesson.
...
- Fail automatically if an ordinary post-overview response exceeds 900 Japanese characters or 350 English words, or uses excessive vertical layout despite short prose, without an explicit request for a longer treatment, self-contained synthesis, or exported note.
...
3. Distinguish the opening map from later turns. Check each later turn's text length and rendered height, including formulas, headings, lists, and the checkpoint; reject both definition-by-definition pacing and attempts to finish oversized milestones in one response. Preserve necessary hypotheses and explanations of unfamiliar terms by narrowing the takeaway, not by removing them.
